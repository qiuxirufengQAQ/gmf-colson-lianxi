...
public class GrGrxxJcTransformer {
	...
	public String transfer(Map<String, Object> data) {
		...
		sb.append(data.get("creditLine")==null?"":(int)Math.ceil(Double.valueOf(data.get("creditLine").toString()))).append(SEPARATOR);//授信额度
		sb.append(data.get("SharedCreditLine")==null?"":(int)Math.ceil(Double.valueOf(data.get("SharedCreditLine").toString()))).append(SEPARATOR);//共享授信额度
		sb.append(data.get("maxLiability")==null?"":(int)Math.ceil(Double.valueOf(data.get("maxLiability").toString()))).append(SEPARATOR);//最大负债额
		...
		sb.append((int)Math.ceil(Double.valueOf(data.get("mustRepayTotalAmount").toString()))).append(SEPARATOR);//本月应还款金额
		sb.append((int)Math.ceil(Double.valueOf(data.get("actualRepayTotalAmount").toString()))).append(SEPARATOR);//本月实际还款金额
		sb.append((int)Math.ceil(Double.valueOf(data.get("balance").toString()))).append(SEPARATOR);//余额
		sb.append(data.get("currentOverdueStage")).append(SEPARATOR);//当前逾期期数
		sb.append((int)Math.ceil(Double.valueOf(data.get("currentOverdueAmount").toString()))).append(SEPARATOR);//当前逾期总额
		sb.append((int)Math.ceil(Double.valueOf(data.get("loanPrincipal31").toString()))).append(SEPARATOR);//逾期31-60天未归还贷款本金
		sb.append((int)Math.ceil(Double.valueOf(data.get("loanPrincipal61").toString()))).append(SEPARATOR);//逾期61-90天未归还贷款本金
		sb.append((int)Math.ceil(Double.valueOf(data.get("loanPrincipal91").toString()))).append(SEPARATOR);//逾期91-180天未归还贷款本金
		sb.append((int)Math.ceil(Double.valueOf(data.get("loanPrincipal180").toString()))).append(SEPARATOR);//逾期180天以上未归还贷款本金
		...
		if ("//////////////////////*C".equals(twoYearAccountRepayStatus1)){
			...
			if(null != data.get("twoYearAccountRepayStatus")){
				...
				if (null != data.get("currentStage") && StringUtils.isNotEmpty(data.get("currentStage").toString())){
					num = Integer.valueOf(data.get("currentStage").toString());
				}
